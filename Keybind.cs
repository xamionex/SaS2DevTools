...
/// <summary>
/// A rebindable input combo: an optional keyboard combo (modifier key + key) and an optional
/// gamepad combo (modifier button + button); either device may be left unbound. Evaluation is
/// stateless (the caller does edge detection). Persisted to a string config entry as
/// "KbMod|KbKey|PadMod|PadButton".
///
/// Kept local to DevTools so it does not depend on the SaS2ModOptions assembly; the string
...
public sealed class Keybind
{
    ...
    /// <summary>
    /// Stateful capture of a key/button combo across frames. Inputs held when capture starts are
    /// ignored until released; the combo commits only once all capture inputs are released, so
    /// holding a key waits for the rest of the combo instead of binding immediately. First input
    /// pressed becomes the modifier, last becomes the key. One session sets whichever device the
    /// user used; the other device's binding is untouched.
    /// </summary>
