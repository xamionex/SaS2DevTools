...
{
    private const int IconId = 60; // ICON_CURSED
    ...
    public static bool MainMenu_Update_Prefix(LevelMainMenu __instance, Character character, float frameTime)
    {
        ...
        switch (icon)
        {
            case IconId:
            {
                ...
                var screen = new LevelDevMenu(__instance.player, returnToScreen: 25);
                DevMenuHelper.AddAndActivateScreen(__instance.player, screen);
                return false;
            }
            case 23:
                __instance.PlayAccept();
    ...
    {
        return InsertBeforeKey(instructions.ToList(), 23, IconId, "Dev Tools");
    }
    ...
    public static bool GameMenu_Update_Prefix(LevelGameMenu __instance, Character character, float frameTime)
    {
        ...
        switch (icon)
        {
            case IconId:
            {
                __instance.PlayAccept();
                __instance.Deactivate();
                var screen = new LevelDevMenu(__instance.player, returnToScreen: 10);
                DevMenuHelper.AddAndActivateScreen(__instance.player, screen);
                return false;
            }
            case 11:
    ...
    {
        return InsertBeforeKey(instructions.ToList(), 11, IconId, "Dev Tools");
    }
